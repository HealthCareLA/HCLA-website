    ...
    <section className="" suppressHydrationWarning>
      ...
      >
        ...
        <div className="max-container relative z-[1]" style={{ paddingTop: '50px' }}>
          {block?.title && (
            <h2 className={`text-center text-blue font-medium mb-[30px] xl:mb-[80px] ${block?.isLargeTitle ? '' : 'text-[32px] lg:text-[42px]'}`}>
              {block.title}
            </h2>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-[30px] lg:gap-[40px] xl:gap-[70px]">
            {block?.points?.map((point, index) => (
              <div className="text-common-clr" key={'m1-point-' + index}>
                {point.icon && (
                  <div className={`mb-[20px] ${point.icon.hasBackground ? 'mb-[10px] bg-[#EEFCFF] w-[90px] h-[90px] p-[15px] rounded-full flex justify-center items-center' : ''}`}>
                    <img
                      src={point.icon.src}
                      alt={point.icon.alt}
                ...
                {point.title && (
                  <h3 className="font-medium mb-[10px] leading-normal text-[22px] lg:text-[24px] xl:text-[28px] text-blue">{point.title}</h3>
                )}
