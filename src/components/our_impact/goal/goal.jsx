        ...
        <div className="max-container relative z-[1]">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-[50px]">
            <div className="w-full lg:max-w-1/2 text-center lg:text-left">
              <h3 className={`font-[500] text-blue mb-[35px] md:mb-[54px] ${block?.isLargeTitle ? 'text-[28px] md:text-[42px] xl:text-[52px] leading-[1.2]' : ''}`}>
                {block?.title}
              </h3>
              {
                block.description && (
              ...
              {
                block.points && (
                  <div className="flex flex-col gap-[30px] text-dark-blue p2 my-[40px]">
                    {
                      block.points?.map((point, index) => (
                        <div key={'ip-goal-' + index} className="flex gap-[8px]">
